...
@section('body')
    <div class="container">
        <div class="d-flex justify-content-center row" style="color: black;">
            <div class="col-md-12">
                <div class="mb-4">
                    <h4>
                        <i class="fas fa-shopping-bag"></i>&nbsp; Award list &nbsp;
                        <a href="#info_form" class="btn btn-primary show-form">Add award</a>
                    </h4>
                    <hr>
                </div>
                @if(session('message'))
                ...
                ?>
                        <div class="card border-success mb-3" style="max-width: 18rem;">
                            <div class="card-header bg-transparent border-success">
                                <h5><b>{{ $item['name']}}</b></h5>
                                <span><i class="fa fa-flag"></i> {{ $item['provide'] }}</span>
                            </div>
                            ...
                            <div class="card-footer bg-transparent border-success">
                                <a href="javascript:delete_item({{ $key }})" class="btn btn-outline-danger btn-sm mt-2" type="button">Delete award</a>
                            </div>
                        </div>
    ...
    <div class="container mt-4" id="info_form" style="display: none;">
        <h4>Add award<button class="btn btn-danger ml-4 close-form">Close</button></h4><hr><br>
        <form action="{{ route('award-info-db') }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="institution-group increment border border-info p-4 my-2">
                <h5>Award Title</h5><hr>
                <div class="form-group">
                    <label for="exampleInputEmail1">Title</label>
                    <input type="text" class="form-control" name="name[]" placeholder="Enter your award name" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label for="exampleInputEmail1">Award provide</label>
                    <input type="text" class="form-control" name="provide[]" placeholder="Enter institution name" required>
                </div>
                <div class="form-group">
    ...
    <script>
        ...
        function delete_item(id){
            ...
            .then((willDelete) => {
                if (willDelete) {
                    swal("award delete succesfully", {
                            icon: "success",
                        })
                        .then(function() {
                            $.ajax({
                                url: '/sl-admin/delete-award/' + id,
                                type: 'get',
